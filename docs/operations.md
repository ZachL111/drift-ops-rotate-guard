...
`drift-ops-rotate-guard` is centered on automation work in JavaScript. The technical target is: Develop a JavaScript command-oriented project for rotate scenarios with round-trip fixtures, lossless normalization checks, and no credentials or hosted services.
